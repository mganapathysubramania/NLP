...
class NaiveBayes:
    ...
    def train(self, data):
        ...
        for i, text in enumerate(data.get_texts("pos")):
            ...
            self.num_positive_reviews = self.num_positive_reviews+1 #counting positive reviews
            for w_c in text.get_words():
                self.count_positive[w_c] = self.count_positive[w_c]+1   #addiing words to dictionary or couning the word occurance
                self.total_positive_words = self.total_positive_words+1 #counting total positive words
        ...
        for i, text in enumerate(data.get_texts("neg")):
            ...
            self.num_negative_reviews = self.num_negative_reviews+1 #counting negative reviews
            for w_c in text.get_words():
                self.count_negative[w_c] = self.count_negative[w_c] + 1 #adding words to count negative dictionary or counting the count of that word
                self.total_negative_words = self.total_negative_words + 1   #counting total neg words
    ...
    def predict(self, data, alpha=1.0):
        ...
        for classval in ["pos", "neg"]:
            for text in data.get_texts(classval):
                ...
                for wordsinfile in text.get_words():

                    if wordsinfile in self.vocab:
                        # finding sum positive
                        sum_positive = sum_positive + log(float(self.count_positive[wordsinfile] + alpha)) \
                                       - log(float(self.total_positive_words + (alpha * len(self.vocab))))
                        #finding sum negative
                        sum_negative = sum_negative + log(float(self.count_negative[wordsinfile] + alpha)) \
                                       - log(float(self.total_negative_words + (alpha * len(self.vocab))))
