...
def parse_sentence(nt_rules, t_rules, words):
    ...
    for LengthofRow in range(1, len(words)): #Took as row and column as per given in example in PDF
        for LengthofColumn in range(0, len(words) - LengthofRow): #Took as row and column as per given in example in PDF
            for Cell in range(LengthofColumn + 1, LengthofColumn + LengthofRow + 1): #Took as cell
                for nonTerminalRule in nt_rules:
                    if chart[Cell - LengthofColumn - 1][LengthofColumn].get(nonTerminalRule.rhs1) is not None and \
                            chart[LengthofColumn + LengthofRow - Cell][Cell].get(nonTerminalRule.rhs2) is not None:
                        chart[LengthofRow][LengthofColumn][nonTerminalRule.lhs] = ParsedPhrase(label=nonTerminalRule.lhs, children=[
                            chart[Cell - LengthofColumn - 1][LengthofColumn][nonTerminalRule.rhs1],
                            chart[LengthofColumn + LengthofRow - Cell][Cell][nonTerminalRule.rhs2]])
